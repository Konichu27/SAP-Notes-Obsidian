![[Pasted image 20260204114202.png]]

![[Pasted image 20260212143817.png]]

![[Pasted image 20260212143923.png]]

![[Pasted image 20260212144230.png]]

![[Pasted image 20260226143952.png]]

![[Pasted image 20260212142455.png]]

Tracks
- ABAP - Development path
- BTP (Business Technology Platform) - integration w/ other applications
- BASIS - administration (hyperscalers etc.)
- Functional consulting
	- FICO - accounting
	- SCM
	- Sales

| **id**   | **age**  | **job**   | **marital** | **education** | **default** | **balance** | **housing** | **loan** | **contact** | **day**  | **month** | **duration** | **campaign** | **pdays** | **previous** | **poutcome** | **y** |
| -------- | -------- | --------- | ----------- | ------------- | ----------- | ----------- | ----------- | -------- | ----------- | -------- | --------- | ------------ | ------------ | --------- | ------------ | ------------ | ----- |
| 1595.076 | 44.82851 | entreprer | married     | primary       | no          | 2075.383    | yes         | no       | unknown     | 14.72658 | may       | 121.8738     | 1.372214     | 4.953944  | -0.063976    | unknown      | no    |

| Jodie Salas | SAP Lead |
| ----------- | -------- |
|             |          |


ALPS - Internal training platform for SAP for ACN
- Learning sessions
- We are ~~Interns~~ Academy Delegates
- Learning is 2-4 hours during onsite
- ALPS is not open during weekends or holidays
**LAPTOP RELEASE**
- To be released first week of March
- At **Uptown 2 BGC**
- SAP Cert must be completed by then
**FUTURE**
- Case study presentation
- Final exam
	- If you fail, you may be reassigned to other Accenture roles (Java, Salesforce, etc.)
	- After that, HR interview -> job offer! :D
**LOCATION**
- Either Mandaluyong or Uptown (BGC)
	- For How to Get There, please put both locations! Since you are likely to change sites throughout your OJT.
- Every **Tuesday**
**BEELINE**
- Internal ACN acct.
**OTHER DETAILS (like immediate supervisor, department, etc.)**
- To be provided in WPR